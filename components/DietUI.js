...
*/
  ...
  const getServingsFor = (person, servingsData) => {
    ...
    return servingsData
      .filter((entry) => {
        ...
        return entryGender === gender && ageNum >= min && ageNum <= max;
      })
      .map((entry) => ({
        fgid: entry.fgid,
  ...
  const getFoodGroups = (servings, foodgroupsData) => {
    return servings.map((diet) => ({
      fgid: diet.fgid,
      groups: foodgroupsData
        .filter((entry) => {
          return entry.fgid == diet.fgid;
        })
        .map((e) => ({
  ...
  const getFoods = (userDiet, foodsData) => {
    return userDiet.map((diet) => ({
      ...diet,
      groups: diet.groups.map((group) => ({
        ...group,

        foods: foodsData.filter((entry) => {
          return entry.fgcat_id == group.fgcat_id;
        }),
      })),
    }));
  };
        ...
        >
          {people.map((person, index) => (
            <TouchableOpacity
              ...
              style={{
                backgroundColor:
                  index === selectedPersonIndex ? "#cce5ff" : "#fff",
                padding: 10,
                borderRadius: 8,
                marginBottom: 10,
              }}
        ...
        <View style={{ flex: 1, backgroundColor: "#fff", padding: 10 }}>
          ...
          >
            {userDiet.map((diet, index) => (
              <TouchableOpacity
                ...
                style={{
                  backgroundColor:
                    selectedFoodGroupKey === index ? "#88cc88" : "#e0e0e0",
                  padding: 10,
                  borderRadius: 20,
                  margin: 10,
                }}
          ...
          <ScrollView>
            <Text style={{ fontSize: 18, fontWeight: "bold", marginBottom: 5 }}>
              ...
              {
                getServingsFor(selectedPerson, servingsData)[
                  selectedFoodGroupKey
                ].servings
              }
            </Text>
