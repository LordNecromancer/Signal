...
import { TouchableOpacity,Text, TextInput, Button, View, StyleSheet, ActivityIndicator, ScrollView, Switch, Platform, Dimensions} from 'react-native';
...
export default function Main() {
  ...
  const fetchUser = async () => {
    ...
    if (session?.session) {
      ...
      const { data, error } = await supabase
        ...
        console.log(data[0])
      if (data && data[0]){
      setUser({ _id: user.id, name: data[0]?.name, sex:data[0]?.sex || 'male' ,age:data[0]?.age|| 25 , family:data[0]?.family || {}});
  ...
  const handleSubmit = async () => {
    ...
    setUser({ _id: user.id, name: payload.name, sex:payload.sex || 'male' ,age:payload.age|| 25 , family:payload.family || {}});
  ...
  return (
    ...
    style={[styles.background, { color: themes[theme].backgroundColor,flex:1 }]}
  >
 ...
 <View style={{ flex: 1, backgroundColor: themes[theme].backgroundColor , marginTop:100}}>
   ...
   {!formSubmitted && (
       ...
       <View style={[styles.form_container, {       width: (Platform.OS !== 'web' || width < 800) ? '90%' : '50%' 
    }]}>
          <Text style={[styles.label,{ color: themes[theme].text }]}>Name:</Text>
    <TextInput
      value={name}
    ...
    <Text style={[styles.label,{ color: themes[theme].text }]}>Age:</Text>
    <TextInput
      value={age}
      onChangeText={setAge}
...
<View style={styles.row}>
  <Text style={[styles.labelSmall,{ color: themes[theme].text }]}>Sex:</Text>
  <Picker
    selectedValue={sex}
    onValueChange={setSex}
    style={[styles.smallPicker, { color: themes[theme].text, backgroundColor:themes[theme].pickerBackgroundColor }]}
  >
  ...
  <Text style={[{ marginRight: 10 , color: themes[theme].text }]}>Include family members?</Text>
  <Switch
    value={includeFamily}
  ...
  {includeFamily && (
    <>
    <Text style={[styles.familyTitle,{ color: themes[theme].text }]}>Family Members</Text>
    {family.map((member, index) => (
      <View key={index} style={styles.familyCard}>
        ...
        <Text style={[styles.memberTitle,{ color: themes[theme].text }]}>Member {index + 1}</Text>
        <TextInput
          placeholder="Name"
          value={member.name}
          onChangeText={text => handleFamilyChange(index, 'name', text)}
          style={[styles.textInput, { color: themes[theme].text }]}
          />
        <TextInput
          placeholder="Age"
          value={member.age}
          onChangeText={text => handleFamilyChange(index, 'age', text)}
          keyboardType="numeric"
          style={[styles.textInput, { color: themes[theme].text }]}
          />
        <View style={styles.row}>
  ...
  <Picker
    ...
    onValueChange={value => handleFamilyChange(index, 'sex', value)}
    style={[styles.smallPicker, { color: themes[theme].text, backgroundColor:themes[theme].pickerBackgroundColor }]}
  >
    <Picker.Item label="Male" value="male" />
    <Picker.Item label="Female" value="female" />
  ...
  <Text style={styles.buttonText}>Add Another Family Member</Text>
